...
\newpage
\chapter*{Выводы}
Основной сложностью моделирования является вычислительная сложность, так как в сумме получается нужно рассчитывать
вероятности для 12404 реакций. Но даже при малом времение работы DEEP удалось подобрать параметры модели, чтобы
концентрации белка \textbf{kni} и мРНК имели схожее поведение с экспериментальными рассчётами.
\newpage
